...
public class QueryFinder {
	...
	public static void main(String[] args) throws IOException {
		...
		workFlow.setPropertyMatcher(new SPARQLPropertyMatcher(END_POINT));
		workFlow.setClassMatcher(new SPARQLClassMatcher(END_POINT));
		workFlow.setResourceMatcher(new SPARQLResourceMatcher(END_POINT));
		...
		for (QueryPattern pattern : query) {
			...
			if (pattern.getPredicate().equals(QueryPattern.IS_A)) {
				// Bind rdf:type
				query.addBinding(pattern.getPredicate(), Node.createURI("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"));
